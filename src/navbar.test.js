...
describe('navbar', () => {
    ...
    test('as tabs estao em ingles', () => {
        ...
        expect(tabs).toEqual(['Catalog', 'Stats', 'Guess', 'Clips']);
    });

    test('a marca fica em portugues: e um nome proprio', () => {
        const { container } = renderNav();
        expect(container.querySelector('.navbar-brand').textContent).toBe('Cantinho do Gueimer');
    });
});
